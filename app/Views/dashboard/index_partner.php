    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modal-title"><i class="fas fa-handshake mr-2"></i>จัดการข้อมูลพาร์ทเนอร์</h4>
            ...
            <form class="mb-3" id="form_review" action="javascript:void(0)" method="post" enctype="multipart/form-data">
                <div class="modal-body">
                    <div >
                        <button class="file-upload-btn" type="button" onclick="$('.file-upload-input').trigger( 'click' )"><i class="fas fa-upload mr-2"></i>เพิ่มรูปภาพ</button>
                        ...
                        <div class="file-upload-content">
                            ...
                            <div class="image-title-wrap">
                                <button type="button" onclick="removeUpload()" class="remove-image"><i class="fas fa-trash mr-2"></i>ลบรูปภาพ <span class="image-title">Uploaded Image</span></button>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-12 mt-5">
                            <div class="form-group">
                                <label for="name_partner"><i class="fas fa-building"></i>ชื่อพาร์ทเนอร์</label>
                                <input type="text" class="form-control" id="name_partner" name="name_partner" placeholder="กรอกชื่อพาร์ทเนอร์" required>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function load_modal(action, data_encode) {
        ...
        switch (action) {
            case 'Create':
                $("#modal-title").html('<i class="fas fa-plus-circle mr-2"></i>เพิ่มพาร์ทเนอร์');
                $('#url_route').val('dashboard/partner/create/' + data_encode);
                check_action = 'Create';
                break;
            case 'Update':
                $("#modal-title").html('<i class="fas fa-edit mr-2"></i>แก้ไขพาร์ทเนอร์');
                const data = JSON.parse(decodeURIComponent(data_encode));
                $('#name_partner').val(data.name_partner);
                $('#url_route').val('dashboard/partner/update/' + data.id_partner);
                $('#path_image_old').val(data.logo_partner_path);
                check_action = 'Update';
                break;
            default:
                break;
                ...
                },
                {
                    ...
                    'render': function(data, type, row, meta) {
                        ...
                        return `<a href="javascript:load_modal('Update', '${encodedRowData}')"><i class="fas fa-edit fa-lg icon-spacing" title="แก้ไขข้อมูล" data-toggle="modal" data-target="#modal-lg"></i></a>
                            ...
                            <a href="javascript:confirm_Alert('ต้องการลบหรือไม่', 'dashboard/partner/delete/${data.id_partner}')"><i class="fas fa-trash fa-lg icon-spacing" title="ลบข้อมูล"></i></a>`;
                    }
                },
            ],
        });
                    ...
                    'columns': [{
                        ...
                        },
                        {
                            ...
                            'render': function(data, type, row, meta) {
                                ...
                                return `<a href="javascript:load_modal('Update', '${encodedRowData}')"><i class="fas fa-edit fa-lg icon-spacing" title="แก้ไขข้อมูล" data-toggle="modal" data-target="#modal-lg"></i></a>
                            <a href="javascript:confirm_Alert('ต้องการเปลี่ยนสถานะหรือไม่', 'dashboard/partner/changestatus/${data.id_partner}/${data.status}')"><i class="fas fa-exchange-alt fa-lg icon-spacing" title="เปลี่ยนสถานะ"></i></a>
                            <a href="javascript:confirm_Alert('ต้องการลบหรือไม่', 'dashboard/partner/delete/${data.id_partner}')"><i class="fas fa-trash icon-spacing" title="ลบข้อมูล"></i></a>`;
                            }
                        },
                    ],
                });
